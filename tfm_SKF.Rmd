...
df_exmat_t <- t(df_exmat)
# Juntamos "df_dataClin" y "df_exmat_t"
df_data_clin_exmat <- cbind(df_dataClin, df_exmat_t)
...
df_hla_nk_expression <- df_data_clin_exmat[hla_nk_genes]
# Añadimos la columna HLA-ABC con la suma de expresión de los 3 genes
df_hla_nk_expression$`HLA-ABC` <- df_hla_nk_expression$`HLA-A` + 
  df_hla_nk_expression$`HLA-B` + df_hla_nk_expression$`HLA-C`
# Calculamos la media de la columna HLA-ABC
#mean_hla_abc <- mean(df_hla_nk_expression$`HLA-ABC`, na.rm = TRUE)
# Calculamos la mediana de la columna HLA-ABC
median_hla_abc <- median(df_hla_nk_expression$`HLA-ABC`, na.rm = TRUE)
...
# Añadimos una nueva columna que indique si el nivel de HLA-ABC es 'Low' o 'High'
df_hla_nk_expression$HLA_ABC_cat <- ifelse(df_hla_nk_expression$`HLA-ABC` < median_hla_abc, 'Low', 'High')
# Convertimos la columna 'HLA_ABC_cat' a factor
df_hla_nk_expression$HLA_ABC_cat <- factor(df_hla_nk_expression$HLA_ABC_cat, levels = c('Low', 'High'))
...
df_hla_nk_expression <- cbind(df_dataClin, df_hla_nk_expression)
```
  
## Gráfico de dispersión  

```{r chunck_HLA_2, fig.width=10, fig.height=10, echo=TRUE}
# Creamos el gráfico de dispersión
pd1 <- ggplot(df_hla_nk_expression, aes(x = `HLA-ABC`, y = NCR1)) + 
    ...
    labs(title = "Gráfico de dispersión NKP46 vs HLA-ABC",
       x = "HLA-ABC",
       y = "NKP46 (NCR1)",
       color = "MSI/MSS",
       shape = "Stage") +
...
```{r chunck_HLA_3, echo=TRUE}
# Ajustamos un modelo lineal
lmod <- lm(NCR1 ~ `HLA-ABC`, data = df_hla_nk_expression)
# Summary del modelo
summary(lmod)
```

- Hay una potencial relación positiva entre HLA-ABC y NCR1, pero no es 
...
pd2 <- ggplot(df_hla_nk_expression_II, aes(x = `HLA-ABC`, y = NCR1)) +
...
lmod_ser <- lm(NCR1 ~ `HLA-ABC`*msi_imputed, data = df_hla_nk_expression_II)
# Hacemos una contraste de paralelismo de rectas con anova
anova(lmod_ser)
```

Vemos que hay significación en la hipótesis de paralelismo (Pr: 0.02811), no son
paralelas, aunque no en la de coincidencia de las rectas (Pr: 0.41378). 
Resultado de difícil interpretación: la significancia de la interacción sugiere 
que el efecto de HLA-ABC sobre NCR1 depende del estado de msi_imputed o 
viceversa. Esto significa que el impacto de una de estas variables en NCR1 no es
constante y varía según el nivel de la otra variable.
...
# Creamos el gráfico de dispersión para Estadio II
pd3 <- ggplot(df_hla_nk_expression_III, aes(x = `HLA-ABC`, y = NCR1)) +
  geom_point(aes(color = msi_imputed, shape = msi_imputed), size = 3, stroke = 1) +
...
# Ajustamos un modelo lineal para Estadio II
lmod_ser <- lm(NCR1 ~ `HLA-ABC`*msi_imputed, data = df_hla_nk_expression_III)
# Hacemos una contraste de paralelismo de rectas con anova
anova(lmod_ser)
```
...
Aquí se intuye una relación inversa entre la expresión de NCR1 y HLA-ABC; es 
decir, una relación inversa entre infiltración de células NK y expresión de 
HLA's.
  
  
## Boxplot HLA-ABC Cat vs NKp46

Boxplot comparando la expresión del gen NKp46 entre las categorías baja (Low) y 
alta (High) de HLA-ABC. Para realizar las categorías  se ha utilizado la mediana
...
library(sqldf)
# Boxplot StageII, MSI
df_hla_nk_expression_II_msi <- sqldf('SELECT * FROM df_hla_nk_expression WHERE stage = "II" AND msi_imputed = "MSI"')
# Realizamos el test de Mann-Whitney U para los grupos Low y High
nLow <- df_hla_nk_expression_II_msi$NCR1[df_hla_nk_expression_II_msi$HLA_ABC_cat == "Low"]
nHigh <- df_hla_nk_expression_II_msi$NCR1[df_hla_nk_expression_II_msi$HLA_ABC_cat == "High"]
mwU_II_msi_test <- wilcox.test(nLow, nHigh, alternative = "two.sided")
# Boxplot
bp31 <- boxplot_MCPcounter(data = df_hla_nk_expression_II_msi, 
                          x=df_hla_nk_expression_II_msi$HLA_ABC_cat, 
                          y=df_hla_nk_expression_II_msi$NCR1, 
                          fill=df_hla_nk_expression_II_msi$HLA_ABC_cat, 
                          title= "StageII, MSI", 
                          p_value=mwU_II_msi_test$p.value, 
                          xlabel="HLA_ABC", ylabel="NKP46")
# Boxplot StageII, MSS
df_hla_nk_expression_II_mss <- sqldf('SELECT * FROM df_hla_nk_expression WHERE stage = "II" AND msi_imputed = "MSS"')
# Realizamos el test de Mann-Whitney U para los grupos Low y High
nLow <- df_hla_nk_expression_II_mss$NCR1[df_hla_nk_expression_II_mss$HLA_ABC_cat == "Low"]
nHigh <- df_hla_nk_expression_II_mss$NCR1[df_hla_nk_expression_II_mss$HLA_ABC_cat == "High"]
mwU_II_mss_test <- wilcox.test(nLow, nHigh, alternative = "two.sided")
# Boxplot
bp32 <- boxplot_MCPcounter(data = df_hla_nk_expression_II_mss, 
                          x=df_hla_nk_expression_II_mss$HLA_ABC_cat, 
                          y=df_hla_nk_expression_II_mss$NCR1, 
                          fill=df_hla_nk_expression_II_mss$HLA_ABC_cat, 
                          title= "StageII, MSS", 
                          p_value=mwU_II_mss_test$p.value, 
                          xlabel="HLA_ABC", ylabel="NKP46")
# Boxplot StageIII, MSI
df_hla_nk_expression_III_msi <- sqldf('SELECT * FROM df_hla_nk_expression WHERE stage = "III" AND msi_imputed = "MSI"')
# Realizamos el test de Mann-Whitney U para los grupos Low y High
nLow <- df_hla_nk_expression_III_msi$NCR1[df_hla_nk_expression_III_msi$HLA_ABC_cat == "Low"]
nHigh <- df_hla_nk_expression_III_msi$NCR1[df_hla_nk_expression_III_msi$HLA_ABC_cat == "High"]
mwU_III_msi_test <- wilcox.test(nLow, nHigh, alternative = "two.sided")
# Boxplot
bp33 <- boxplot_MCPcounter(data = df_hla_nk_expression_III_msi, 
                          x=df_hla_nk_expression_III_msi$HLA_ABC_cat, 
                          y=df_hla_nk_expression_III_msi$NCR1, 
                          fill=df_hla_nk_expression_III_msi$HLA_ABC_cat, 
                          title= "StageIII, MSI", 
                          p_value=mwU_III_msi_test$p.value, 
                          xlabel="HLA_ABC", ylabel="NKP46")
# Boxplot StageIII, MSS
df_hla_nk_expression_III_mss <- sqldf('SELECT * FROM df_hla_nk_expression WHERE stage = "III" AND msi_imputed = "MSS"')
# Realizamos el test de Mann-Whitney U para los grupos Low y High
nLow <- df_hla_nk_expression_III_mss$NCR1[df_hla_nk_expression_III_mss$HLA_ABC_cat == "Low"]
nHigh <- df_hla_nk_expression_III_mss$NCR1[df_hla_nk_expression_III_mss$HLA_ABC_cat == "High"]
mwU_III_mss_test <- wilcox.test(nLow, nHigh, alternative = "two.sided")
# Boxplot
bp34 <- boxplot_MCPcounter(data = df_hla_nk_expression_III_mss, 
                          x=df_hla_nk_expression_III_mss$HLA_ABC_cat, 
                          y=df_hla_nk_expression_III_mss$NCR1, 
                          fill=df_hla_nk_expression_III_mss$HLA_ABC_cat, 
                          title= "StageIII, MSS", 
                          p_value=mwU_III_mss_test$p.value, 
                          xlabel="HLA_ABC", ylabel="NKP46")

# Visualizamos los gráficos
require(patchwork)
# En un grid 2x2
bp31 + bp32 + bp33 + bp34 +plot_layout(ncol = 2, nrow = 2)
```

No da significación en ningún caso. 
No hay una diferencia signficativa en la mediana de la expresión del gen NCR1 
entre los grupos con baja y alta expresión de HLA-ABC.
...
df_hla_nk_expression_II_msi <- sqldf('SELECT * FROM df_hla_nk_expression WHERE stage = "II" AND msi_imputed = "MSI"')
# Realizamos el test de Mann-Whitney U para los grupos Low y High
nLow <- df_hla_nk_expression_II_msi$NCR1[df_hla_nk_expression_II_msi$HLA_A_cat == "Low"]
nHigh <- df_hla_nk_expression_II_msi$NCR1[df_hla_nk_expression_II_msi$HLA_A_cat == "High"]
mwU_II_msi_test <- wilcox.test(nLow, nHigh, alternative = "two.sided")
# Boxplot
bp35 <- boxplot_MCPcounter(data = df_hla_nk_expression_II_msi, 
                          x=df_hla_nk_expression_II_msi$HLA_A_cat, 
                          y=df_hla_nk_expression_II_msi$NCR1, 
                          fill=df_hla_nk_expression_II_msi$HLA_A_cat, 
                          title= "StageII, MSI", 
                          p_value=mwU_II_msi_test$p.value, 
                          xlabel="HLA_A", ylabel="NKP46")
# Boxplot StageII, MSS
df_hla_nk_expression_II_mss <- sqldf('SELECT * FROM df_hla_nk_expression WHERE stage = "II" AND msi_imputed = "MSS"')
# Realizamos el test de Mann-Whitney U para los grupos Low y High
nLow <- df_hla_nk_expression_II_mss$NCR1[df_hla_nk_expression_II_mss$HLA_A_cat == "Low"]
nHigh <- df_hla_nk_expression_II_mss$NCR1[df_hla_nk_expression_II_mss$HLA_A_cat == "High"]
mwU_II_mss_test <- wilcox.test(nLow, nHigh, alternative = "two.sided")
# Boxplot
bp36 <- boxplot_MCPcounter(data = df_hla_nk_expression_II_mss, 
                          x=df_hla_nk_expression_II_mss$HLA_A_cat, 
                          y=df_hla_nk_expression_II_mss$NCR1, 
                          fill=df_hla_nk_expression_II_mss$HLA_A_cat, 
                          title= "StageII, MSS", 
                          p_value=mwU_II_mss_test$p.value, 
                          xlabel="HLA_A", ylabel="NKP46")
# Boxplot StageIII, MSI
df_hla_nk_expression_III_msi <- sqldf('SELECT * FROM df_hla_nk_expression WHERE stage = "III" AND msi_imputed = "MSI"')
# Realizamos el test de Mann-Whitney U para los grupos Low y High
nLow <- df_hla_nk_expression_III_msi$NCR1[df_hla_nk_expression_III_msi$HLA_A_cat == "Low"]
nHigh <- df_hla_nk_expression_III_msi$NCR1[df_hla_nk_expression_III_msi$HLA_A_cat == "High"]
mwU_III_msi_test <- wilcox.test(nLow, nHigh, alternative = "two.sided")
# Boxplot
bp37 <- boxplot_MCPcounter(data = df_hla_nk_expression_III_msi, 
                          x=df_hla_nk_expression_III_msi$HLA_A_cat, 
                          y=df_hla_nk_expression_III_msi$NCR1, 
                          fill=df_hla_nk_expression_III_msi$HLA_A_cat, 
                          title= "StageIII, MSI", 
                          p_value=mwU_III_msi_test$p.value, 
                          xlabel="HLA_A", ylabel="NKP46")
# Boxplot StageIII, MSS
df_hla_nk_expression_III_mss <- sqldf('SELECT * FROM df_hla_nk_expression WHERE stage = "III" AND msi_imputed = "MSS"')
# Realizamos el test de Mann-Whitney U para los grupos Low y High
nLow <- df_hla_nk_expression_III_mss$NCR1[df_hla_nk_expression_III_mss$HLA_A_cat == "Low"]
nHigh <- df_hla_nk_expression_III_mss$NCR1[df_hla_nk_expression_III_mss$HLA_A_cat == "High"]
mwU_III_mss_test <- wilcox.test(nLow, nHigh, alternative = "two.sided")
# Boxplot
bp38 <- boxplot_MCPcounter(data = df_hla_nk_expression_III_mss, 
                          x=df_hla_nk_expression_III_mss$HLA_A_cat, 
                          y=df_hla_nk_expression_III_mss$NCR1, 
                          fill=df_hla_nk_expression_III_mss$HLA_A_cat, 
                          title= "StageIII, MSS", 
                          p_value=mwU_III_mss_test$p.value, 
                          xlabel="HLA_A", ylabel="NKP46")
...
df_hla_nk_expression_II_msi <- sqldf('SELECT * FROM df_hla_nk_expression WHERE stage = "II" AND msi_imputed = "MSI"')
# Realizamos el test de Mann-Whitney U para los grupos Low y High
nLow <- df_hla_nk_expression_II_msi$NCR1[df_hla_nk_expression_II_msi$HLA_E_cat == "Low"]
nHigh <- df_hla_nk_expression_II_msi$NCR1[df_hla_nk_expression_II_msi$HLA_E_cat == "High"]
mwU_II_msi_test <- wilcox.test(nLow, nHigh, alternative = "two.sided")
# Boxplot
bp39 <- boxplot_MCPcounter(data = df_hla_nk_expression_II_msi, 
                          x=df_hla_nk_expression_II_msi$HLA_E_cat, 
                          y=df_hla_nk_expression_II_msi$NCR1, 
                          fill=df_hla_nk_expression_II_msi$HLA_E_cat, 
                          title= "StageII, MSI", 
                          p_value=mwU_II_msi_test$p.value, 
                          xlabel="HLA_E", ylabel="NKP46")
# Boxplot StageII, MSS
df_hla_nk_expression_II_mss <- sqldf('SELECT * FROM df_hla_nk_expression WHERE stage = "II" AND msi_imputed = "MSS"')
# Realizamos el test de Mann-Whitney U para los grupos Low y High
nLow <- df_hla_nk_expression_II_mss$NCR1[df_hla_nk_expression_II_mss$HLA_E_cat == "Low"]
nHigh <- df_hla_nk_expression_II_mss$NCR1[df_hla_nk_expression_II_mss$HLA_E_cat == "High"]
mwU_II_mss_test <- wilcox.test(nLow, nHigh, alternative = "two.sided")
# Boxplot
bp40 <- boxplot_MCPcounter(data = df_hla_nk_expression_II_mss, 
                          x=df_hla_nk_expression_II_mss$HLA_E_cat, 
                          y=df_hla_nk_expression_II_mss$NCR1, 
                          fill=df_hla_nk_expression_II_mss$HLA_E_cat, 
                          title= "StageII, MSS", 
                          p_value=mwU_II_mss_test$p.value, 
                          xlabel="HLA_E", ylabel="NKP46")
# Boxplot StageIII, MSI
df_hla_nk_expression_III_msi <- sqldf('SELECT * FROM df_hla_nk_expression WHERE stage = "III" AND msi_imputed = "MSI"')
# Realizamos el test de Mann-Whitney U para los grupos Low y High
nLow <- df_hla_nk_expression_III_msi$NCR1[df_hla_nk_expression_III_msi$HLA_E_cat == "Low"]
nHigh <- df_hla_nk_expression_III_msi$NCR1[df_hla_nk_expression_III_msi$HLA_E_cat == "High"]
mwU_III_msi_test <- wilcox.test(nLow, nHigh, alternative = "two.sided")
# Boxplot
bp41 <- boxplot_MCPcounter(data = df_hla_nk_expression_III_msi, 
                          x=df_hla_nk_expression_III_msi$HLA_E_cat, 
                          y=df_hla_nk_expression_III_msi$NCR1, 
                          fill=df_hla_nk_expression_III_msi$HLA_E_cat, 
                          title= "StageIII, MSI", 
                          p_value=mwU_III_msi_test$p.value, 
                          xlabel="HLA_E", ylabel="NKP46")
# Boxplot StageIII, MSS
df_hla_nk_expression_III_mss <- sqldf('SELECT * FROM df_hla_nk_expression WHERE stage = "III" AND msi_imputed = "MSS"')
# Realizamos el test de Mann-Whitney U para los grupos Low y High
nLow <- df_hla_nk_expression_III_mss$NCR1[df_hla_nk_expression_III_mss$HLA_E_cat == "Low"]
nHigh <- df_hla_nk_expression_III_mss$NCR1[df_hla_nk_expression_III_mss$HLA_E_cat == "High"]
mwU_III_mss_test <- wilcox.test(nLow, nHigh, alternative = "two.sided")
# Boxplot
bp42 <- boxplot_MCPcounter(data = df_hla_nk_expression_III_mss, 
                          x=df_hla_nk_expression_III_mss$HLA_E_cat, 
                          y=df_hla_nk_expression_III_mss$NCR1, 
                          fill=df_hla_nk_expression_III_mss$HLA_E_cat, 
                          title= "StageIII, MSS", 
                          p_value=mwU_III_mss_test$p.value, 
                          xlabel="HLA_E", ylabel="NKP46")
...
MSI/MSS-Estadio|`r msi_stage_Accuracy`|`r msi_stage_Kappa`|(`r msi_stage_AccuracyLower` - `r msi_stage_AccuracyUpper`)
