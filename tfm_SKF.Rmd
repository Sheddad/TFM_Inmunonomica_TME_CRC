...
boxplot_MCPcounter <- function(data, x, y, fill, title, p_value=NULL) {
  ...
  bp <- ggplot(data, aes_string(x = x, y = y, fill = fill)) +
    ...
    if (!is.null(p_value)) {
      bp <- bp + annotate("text", x = Inf, y = Inf, label = sprintf("K-W p-value: %.3f", p_value), 
                          vjust = 0.95, hjust = 2.03, size = 4, color = "turquoise3")
  }
...
df_dataClin$grupo <- with(df_dataClin, paste(msi_imputed, stage, sep = " s "))
...
```{r chunck_3_5, fig.width=8, fig.height=8,}
...
# Estimación de la infiltración celular 

Analizamos los datos viendo la infiltración celular.  

## MCPcounter {.tabset .tabset-fade .tabset-pills}
...
```{r chunck_4_1_2_0, echo=TRUE}
# Calculamos los test no paraméticos Kruskal-Wallis (asumimos que no tenemos normalidad) 16/04/2024
# Preparar datos (asegurarte de que `grupo` y `exmat_MCP_Clin$'NK cells'` están correctamente definidos)
data <- exmat_MCP_Clin

# Realizar test de Kruskal-Wallis
kruskal_result <- kruskal.test(exmat_MCP_Clin$`Endothelial cells` ~ grupo, data = data)
print(kruskal_result)
```


```{r chunck_4_1_2, fig.width=10, fig.height=5, echo=TRUE}
# Generamos todos los boxplot 27/03/2024
grupo <- exmat_MCP_Clin$grupo
# NK
bp1 <- boxplot_MCPcounter (exmat_MCP_Clin, grupo, exmat_MCP_Clin$`NK cells`, grupo, "NK cells", 0.9)
# T cells
bp2 <- boxplot_MCPcounter (exmat_MCP_Clin, grupo, exmat_MCP_Clin$`T cells`, grupo, "T cells")
# CD8 T cells
bp3 <- boxplot_MCPcounter (exmat_MCP_Clin, grupo, exmat_MCP_Clin$`CD8 T cells`, grupo, "CD8 T cells")
# Cytotoxic lymphocytes
bp4 <- boxplot_MCPcounter (exmat_MCP_Clin, grupo, exmat_MCP_Clin$`Cytotoxic lymphocytes`, grupo, "Cytotoxic lymphocytes")
# B lineage
bp5 <- boxplot_MCPcounter (exmat_MCP_Clin, grupo, exmat_MCP_Clin$`B lineage`, grupo, "B lineage")
# Monocytic lineage
bp6 <- boxplot_MCPcounter (exmat_MCP_Clin, grupo, exmat_MCP_Clin$`Monocytic lineage`, grupo, "Monocytic lineage")
# Myeloid dendritic cells
bp7 <- boxplot_MCPcounter (exmat_MCP_Clin, grupo, exmat_MCP_Clin$`Myeloid dendritic cells`, grupo, "Myeloid dendritic cells")
# Neutrophils
bp8 <- boxplot_MCPcounter (exmat_MCP_Clin, grupo, exmat_MCP_Clin$Neutrophils, grupo, "Neutrophils")
# Endothelial cells
bp9 <- boxplot_MCPcounter (exmat_MCP_Clin, grupo, exmat_MCP_Clin$`Endothelial cells`, grupo, "Endothelial cells")
# Fibroblasts
bp10 <- boxplot_MCPcounter (exmat_MCP_Clin, grupo, exmat_MCP_Clin$Fibroblasts, grupo, "Fibroblasts")
...
```{r chunck_4_1_3, fig.width=9, fig.height=13, echo=TRUE}
...
```{r chunck_4_3_2, fig.width=10, fig.height=5, echo=TRUE}
...
# NK
bp1 <- boxplot_MCPcounter (exmat_MCP_Clin_CMS, cms, exmat_MCP_Clin_CMS$`NK cells`, cms, "NK cells")
# T cells
bp2 <- boxplot_MCPcounter (exmat_MCP_Clin_CMS, cms, exmat_MCP_Clin_CMS$`T cells`, cms, "T cells")
# CD8 T cells
bp3 <- boxplot_MCPcounter (exmat_MCP_Clin_CMS, cms, exmat_MCP_Clin_CMS$`CD8 T cells`, cms, "CD8 T cells")
# Cytotoxic lymphocytes
bp4 <- boxplot_MCPcounter (exmat_MCP_Clin_CMS, cms, exmat_MCP_Clin_CMS$`Cytotoxic lymphocytes`, cms, "Cytotoxic lymphocytes")
# B lineage
bp5 <- boxplot_MCPcounter (exmat_MCP_Clin_CMS, cms, exmat_MCP_Clin_CMS$`B lineage`, cms, "B lineage")
# Monocytic lineage
bp6 <- boxplot_MCPcounter (exmat_MCP_Clin_CMS, cms, exmat_MCP_Clin_CMS$`Monocytic lineage`, cms, "Monocytic lineage")
# Myeloid dendritic cells
bp7 <- boxplot_MCPcounter (exmat_MCP_Clin_CMS, cms, exmat_MCP_Clin_CMS$`Myeloid dendritic cells`, cms, "Myeloid dendritic cells")
# Neutrophils
bp8 <- boxplot_MCPcounter (exmat_MCP_Clin_CMS, cms, exmat_MCP_Clin_CMS$Neutrophils, cms, "Neutrophils")
# Endothelial cells
bp9 <- boxplot_MCPcounter (exmat_MCP_Clin_CMS, cms, exmat_MCP_Clin_CMS$`Endothelial cells`, cms, "Endothelial cells")
# Fibroblasts
bp10 <- boxplot_MCPcounter (exmat_MCP_Clin_CMS, cms, exmat_MCP_Clin_CMS$Fibroblasts, cms, "Fibroblasts")
